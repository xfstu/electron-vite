...
(function () {
  ...
  Select.defineClause(
    'limit',
    function (opts) { if (this._limit != null) return `LIMIT ${sql._handleValue(this._limit, opts)}`; },
    { after: 'orderBy' }
  );

  Select.defineClause(
    'offset',
    function (opts) { if (this._offset != null) return `OFFSET ${sql._handleValue(this._offset, opts)}`; },
    { after: 'limit' }
  );
